  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-medium">目標管理</h1>
      </div>
      ...
      {members.length === 0 && (
        <p className="text-sm text-orange-500 mb-4">
          先に <Link href="/members" className="underline">メンバー管理</Link> でメンバーを追加してください
        </p>
      )}

      {/* 目標一覧 */}
      {Object.keys(grouped).length === 0 ? (
        <p className="text-sm text-gray-400">この期間の目標はまだありません</p>
      ) : (
                    ...
                    {editingId === g.id ? (
                      <div className="flex gap-2 flex-1">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={saveEdit} className="px-3 py-1 bg-blue-600 text-white rounded-lg text-xs">保存</button>
                      ...
                      <>
                        ...
                        <div className="flex gap-3">
                          <button onClick={() => startEdit(g)} className="text-xs text-gray-400 hover:text-blue-500">編集</button>
